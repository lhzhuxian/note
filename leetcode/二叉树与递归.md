...
class Solution {
  public Node inorderSuccessor(Node x) {
    ...
    // 要一直找到一个parent，它是它的parent的左子树，这个是第一个比node x 大的node
...
class Solution {
    public boolean isCompleteTree(TreeNode root) {
        ...
        bfs.offer(root);
      // level traversal 直到碰到null为止
        while (bfs.peek() != null) {
            TreeNode node = bfs.poll();
            bfs.offer(node.left);
            bfs.offer(node.right);
        }
      // 如果 tree是complete的话，queue里面的node 应该都是null
...
class Solution {
     ...
     public TreeNode subtreeWithAllDeepest(TreeNode root) {
        return deep(root, 0).getValue();
    }

    public Pair<Integer, TreeNode> deep(TreeNode root, int cur) {
        if (root == null) return new Pair(cur, null);
        Pair<Integer, TreeNode> l = deep(root.left, cur + 1), r = deep(root.right, cur + 1);

        int d1 = l.getKey(), d2 = r.getKey();
        
        int deepest = Math.max(d1, d2);
        TreeNode node = null;
        if(d1 == d2) {
            node = root;
        } else if(d1 > d2) {
...
dfs去找p, q 如果找到了就直接返回，如果没有接继续从左右子树找，左右子树返回的值如果都是非null，那说明，p, q在当前root的左右两边，返回当前root
...
不同点是 原来是garantee p, q都存在，现在我们需要dfs整个tree才能确定是不是有lca，并且要记住是否碰到了pq
